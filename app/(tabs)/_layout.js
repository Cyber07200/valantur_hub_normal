...
function JellyTabIndicator({ activeIndex }) {
  ...
  useEffect(() => {
    Animated.sequence([
      Animated.timing(scaleX, { toValue: 0.85, duration: 150, easing: Easing.out(Easing.ease), useNativeDriver: true }),
      Animated.parallel([
        Animated.timing(translateX, { toValue: activeIndex * TAB_WIDTH, duration: 250, easing: Easing.inOut(Easing.ease), useNativeDriver: true }),
        Animated.sequence([
          Animated.timing(scaleX, { toValue: 1.1, duration: 150, easing: Easing.out(Easing.ease), useNativeDriver: true }),
          Animated.spring(scaleX, { toValue: 1, friction: 3, tension: 100, useNativeDriver: true }),
        ]),
      ]),
    ]).start();
  }, [activeIndex]);

  return <Animated.View style={[styles.jellyIndicator, { transform: [{ translateX }, { scaleX }] }]} />;
}
...
function JellyIcon({ isActive, children }) {
  ...
  const rotateInterpolate = rotate.interpolate({ inputRange: [0, 1], outputRange: ['0deg', '15deg'] });
  return <Animated.View style={{ transform: [{ scale }, { rotate: rotateInterpolate }] }}>{children}</Animated.View>;
}
...
function CustomTabBar({ state, descriptors, navigation }) {
  ...
  return (
    <View style={[styles.tabBarContainer, { backgroundColor: colors.surface, borderTopColor: colors.border }]}>
      <View style={styles.topLine}><View style={[styles.topLineInner, { backgroundColor: colors.primary }]} /></View>
      <View style={styles.tabsRow}>
        <View style={styles.indicatorContainer}><JellyTabIndicator activeIndex={state.index} /></View>
        {state.routes.map((route, index) => {
          ...
          return (
            ...
            }} style={styles.tabButton} activeOpacity={0.7}>
              <JellyIcon isActive={isFocused}>{getIcon(route.name, isFocused ? colors.primary : colors.textSecondary, 22)}</JellyIcon>
              <Text style={[styles.tabLabel, { color: isFocused ? colors.primary : colors.textSecondary, fontWeight: isFocused ? '700' : '500' }]}>
                {route.name === 'index' ? 'Поиск' : route.name === 'bookings' ? 'Мои записи' : 'Профиль'}
              </Text>
            </TouchableOpacity>
          );
        })}
...
const styles = StyleSheet.create({
  tabBarContainer: { paddingTop: 8, paddingBottom: 30, paddingHorizontal: 10, borderTopLeftRadius: 20, borderTopRightRadius: 20, borderTopWidth: 1, shadowColor: '#000', shadowOffset: { width: 0, height: -4 }, shadowOpacity: 0.08, shadowRadius: 12, elevation: 10 },
  ...
  jellyIndicator: { position: 'absolute', top: 0, left: 0, width: TAB_WIDTH, height: '100%', backgroundColor: 'rgba(99,102,241,0.1)', borderRadius: 16, borderWidth: 2, borderColor: 'rgba(99,102,241,0.3)' },
